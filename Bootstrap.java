...
public class Bootstrap implements Serializable  {
	...
	String lookup(int key) throws UnknownHostException, IOException, ClassNotFoundException {
		...
		//System.out.println(nsDetails.getSuccessorAddress());
	//	System.out.println(nsDetails.sPort);
		 fwdSocket = new Socket(nsDetails.getSuccessorAddress(), nsDetails.sPort);
	...
	try {
		...
		if(args[0].isEmpty())
		{
			System.out.print("Config file name not provided. Please provide it with the command line arguement");
			return;
		}
		sPortConnection = 0;
		int id = 0;
		...
		//read data from config file 	
			...
			sPortConnection = Integer.parseInt(sc.nextLine());
			server = new ServerSocket(sPortConnection);//server for listing to new name server
		    ...
		    while(true) {
				...
				case "entry":
					...
					System.out.println("Hello"+bootstrap.nsDetails.sPortConnection);
					System.out.println("Hello"+bootstrap.nsDetails.id);
					if(bootstrap.nsDetails.getSuccessorId() == 0)//if only one server intial
					{
						outputStream.writeObject(bootstrap.nsDetails.sPortConnection);//succssor port
						outputStream.writeObject(bootstrap.nsDetails.sPortConnection);//predessor port
						outputStream.writeObject(bootstrap.nsDetails.id);//sucessor id
						outputStream.writeObject(bootstrap.nsDetails.id);//predessor id
						...
						bootstrap.nsDetails.updateInfo(newNSListeningPort, newNSListeningPort, newNSId, newNSId, newNSIP, newNSIP);
						...
						bootstrap.nsDetails.pId = newNSId; //update predessor of bootstrap
						
						int nextServerListeningPort = bootstrap.nsDetails.sPort;
						String nextServerIP = bootstrap.nsDetails.getSuccessorAddress();
						...
						fwdSocket = new Socket(nextServerIP, nextServerListeningPort);
						 ...
						 int sPort = (int) inputStreamFwd.readObject();
						 outputStream.writeObject(sPort);//send successor port
						 int pPort = (int) inputStreamFwd.readObject();
						 outputStream.writeObject(pPort);//send predessor port
						 int sId = (int) inputStreamFwd.readObject();
						 outputStream.writeObject(sId);//send successor id
						 int pId = (int) inputStreamFwd.readObject();
						 outputStream.writeObject(pId);//send predessor id
						 String sAddress = (String) inputStreamFwd.readObject();	
						 outputStream.writeObject(sAddress);//send sucessor ip
						 String pAddress = (String) inputStreamFwd.readObject();	
						 outputStream.writeObject(pAddress);//send predessor ip
						...
						//1)if new nameserver is between current server and successor and update the successor of current server
						 if(bootstrap.nsDetails.getSuccessorId() > newNSId)
							 bootstrap.nsDetails.updateInfo(newNSListeningPort,bootstrap.nsDetails.pPort, newNSId, bootstrap.nsDetails.pId, newNSIP,bootstrap.nsDetails.pAddress);
						...
						//2) if new server is not in between,or even if it is between contact the next nameserver
						 ...
						 outputStream.writeObject(pId);//send predessor id
						 String sAddress = (String) inputStreamFwd.readObject();	
						 outputStream.writeObject(sAddress);//send successor ip
						 String pAddress = (String) inputStreamFwd.readObject();	
						 outputStream.writeObject(pAddress);//send predessor ip
						while(true) {
				...
				case "updateYourPredessorAndTakeAllKeys":
					...
					int pPort = (int) inputStream.readObject();//update successor port
					int pId = (int) inputStream.readObject();//update successor id
					String pAddress = (String) inputStream.readObject();//update successor ip
					bootstrap.nsDetails.updateInfo(bootstrap.nsDetails.sPort, pPort, bootstrap.nsDetails.getSuccessorId(), pId,bootstrap.nsDetails.sAddress,pAddress);
					while(true) {
					...
					System.out.println("Updated Informatio sId" + bootstrap.nsDetails.sId);
					break;
				...
				case "updateYourSuccessor":
					...
					int sId = (int) inputStream.readObject();//update predessor id
					String sAddress = (String) inputStream.readObject();//update predessor ip
					bootstrap.nsDetails.updateInfo(successorPort, bootstrap.nsDetails.pPort,sId, bootstrap.nsDetails.pId, sAddress,bootstrap.nsDetails.pAddress);
				break;
				...
				System.out.println("BOOTSTRAP SuccessorId : "+bootstrap.nsDetails.sId + " PredessorId :"+bootstrap.nsDetails.pId);
